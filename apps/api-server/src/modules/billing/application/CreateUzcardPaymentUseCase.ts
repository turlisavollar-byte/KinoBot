...
export interface CreateUzcardPaymentDTO {
  invoiceId: string;
  returnUrl?: string;
}

@injectable()
export class CreateUzcardPaymentUseCase {
  constructor(
  ...
  async execute(dto: CreateUzcardPaymentDTO): Promise<{ paymentUrl: string }> {
    ...
    const paymentUrl = this.uzcardService.generatePaymentUrl({
