...
export function analyticsCacheMiddleware(prefix: string, ttl: number = 300) {
  return async (
    req: Request,
    res: Response,
    next: NextFunction,
    ...
    try {
      ...
      if (cached) {
        ...
        res.json({
          success: true,
          data:
            cached.success !== undefined && "data" in cached
              ? cached.data
              : cached,
          meta: {
            cached: true,
      ...
      res.json = function (data: any) {
        ...
        if (data.success !== false) {
          cacheService.set(key, data.data ?? data, ttl).catch((err) => {
