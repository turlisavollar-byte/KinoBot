...
export class DrizzleCatalogRepository implements ICatalogRepository {
  ...
  async getMovieById(id: string): Promise<Movie | null> {
    const [movie] = await db
      .select()
      .from(moviesTable)
      .where(and(eq(moviesTable.id, id), sql`${moviesTable.deletedAt} IS NULL`))
      .limit(1);
  ...
  async createMovie(
    data: Omit<Movie, "id" | "createdAt" | "updatedAt" | "deletedAt"> & {
      genreIds?: number[];
    },
  ): Promise<Movie> {
    ...
    if (genreIds && genreIds.length > 0) {
      await db
        .insert(movieGenresTable)
        .values(genreIds.map((gid) => ({ movieId: movie.id, genreId: gid })));
    }
  ...
  async updateMovie(
    id: string,
    data: Partial<Movie> & { genreIds?: number[] },
  ): Promise<Movie> {
    ...
    await db
      ...
      .set(updateData)
      .where(
        and(eq(moviesTable.id, id), sql`${moviesTable.deletedAt} IS NULL`),
      );
    ...
    if (genreIds !== undefined) {
      ...
      if (genreIds.length > 0) {
        await db
          .insert(movieGenresTable)
          .values(genreIds.map((gid) => ({ movieId: id, genreId: gid })));
      }
    }
  ...
  async listSeries(params: {
    ...
    if (isPublished !== undefined)
      conditions.push(eq(seriesTable.isPublished, isPublished));
  ...
  async createSeries(
    data: Omit<Series, "id" | "createdAt" | "updatedAt" | "deletedAt"> & {
      genreIds?: number[];
    },
  ): Promise<Series> {
    ...
    if (genreIds?.length) {
      await db
        .insert(seriesGenresTable)
        .values(genreIds.map((gid) => ({ seriesId: series.id, genreId: gid })));
    }
  ...
  async updateSeries(
    id: string,
    data: Partial<Series> & { genreIds?: number[] },
  ): Promise<Series> {
    const { genreIds, ...updateData } = data;
    await db
      .update(seriesTable)
      .set(updateData)
      .where(
        and(eq(seriesTable.id, id), sql`${seriesTable.deletedAt} IS NULL`),
      );
    if (genreIds !== undefined) {
      await db
        .delete(seriesGenresTable)
        .where(eq(seriesGenresTable.seriesId, id));
      if (genreIds.length > 0)
        await db
          .insert(seriesGenresTable)
          .values(genreIds.map((gid) => ({ seriesId: id, genreId: gid })));
    }
  ...
  async createSeason(
    seriesId: string,
    data: Omit<Season, "id" | "createdAt" | "deletedAt">,
  ): Promise<Season> {
  ...
  async listEpisodes(seasonId: string): Promise<Episode[]> {
    ...
    return episodes.map((e) => ({
      ...e,
      viewsCount: Number(e.viewsCount),
    })) as Episode[];
  }

  async createEpisode(
    seasonId: string,
    data: Omit<Episode, "id" | "createdAt" | "updatedAt" | "deletedAt">,
  ): Promise<Episode> {
    const [ep] = await db
  ...
  async getGenreById(id: number): Promise<Genre | null> {
    const [genre] = await db
      .select()
      .from(genresTable)
      .where(and(eq(genresTable.id, id), sql`${genresTable.deletedAt} IS NULL`))
      .limit(1);
    return genre || null;
  }
  ...
  async deleteGenre(id: number): Promise<void> {
    await db
      ...
      .set({ deletedAt: new Date() })
      .where(
        and(eq(genresTable.id, id), sql`${genresTable.deletedAt} IS NULL`),
      );
  }

  // ─── ACTORS ───────────────────────────────────────────────────────────────────
  async listActors(params: { page?: number; limit?: number; search?: string }) {
    ...
    return { data: actors, total: count, page, limit };
  }
  ...
  async deleteActor(id: string): Promise<void> {
    await db
      ...
      .set({ deletedAt: new Date() })
      .where(
        and(eq(actorsTable.id, id), sql`${actorsTable.deletedAt} IS NULL`),
      );
  }
}
