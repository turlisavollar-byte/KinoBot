...
export const genresTable = pgTable(
  ...
  (t) => [
    uniqueIndex("genres_name_active_unique_idx")
      .on(t.name)
      .where(sql`deleted_at IS NULL`),
    uniqueIndex("genres_slug_active_unique_idx")
      .on(t.slug)
      .where(sql`deleted_at IS NULL`),
  ],
);
...
export const moviesTable = pgTable(
  ...
  (t) => [
    index("movies_title_trgm_idx").using("gin", sql`${t.title} gin_trgm_ops`),
    index("movies_catalog_idx")
      .on(t.isPublished, t.createdAt)
      .where(sql`deleted_at IS NULL`),
  ],
);
...
export const movieActorsTable = pgTable(
  "movie_actors",
  {
    movieId: text("movie_id")
      .notNull()
...
export const seasonsTable = pgTable(
  ...
  (t) => [
    uniqueIndex("seasons_series_number_active_unique_idx")
      .on(t.seriesId, t.seasonNumber)
      .where(sql`deleted_at IS NULL`),
    index("seasons_series_active_idx")
      .on(t.seriesId, t.seasonNumber)
      .where(sql`deleted_at IS NULL`),
  ],
);
...
export const episodesTable = pgTable(
  ...
  (t) => [
    uniqueIndex("episodes_season_number_active_unique_idx")
      .on(t.seasonId, t.episodeNumber)
      .where(sql`deleted_at IS NULL`),
    index("episodes_season_active_idx")
      .on(t.seasonId, t.episodeNumber)
      .where(sql`deleted_at IS NULL`),
  ],
);
